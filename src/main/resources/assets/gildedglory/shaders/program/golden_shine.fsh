...
vec4 gatherSamplesRed(vec2 uv, sampler2D tex, float spread) {
    ...
    vec4 base = texture(tex, uv);
    base.rgb = recoverColor(base.rgb);
    vec4 result = base * darkAffinity(base.rgb);
    ...
    for (float i = 1.0; i <= BLOOM_SAMPLES; i += 1.0) {
        ...
        result += s1 * darkAffinity(s1.rgb);
        result += s2 * darkAffinity(s2.rgb);
        result += s3 * darkAffinity(s3.rgb);
        result += s4 * darkAffinity(s4.rgb);
    }

    return (result) / (8.0 * BLOOM_SAMPLES);
}
