...
vec4 blend(vec2 Coord, float MipBias) {
    ...
    for (float i = 1.0; i <= maxSamples; i ++) {
        float inv = 1.0 / i;
        vec4 sample1 = texture(EntitySampler, Coord + vec2(TexelSize.x, TexelSize.y) * inv);
        vec4 sample2 = texture(EntitySampler, Coord + vec2(-TexelSize.x, TexelSize.y) * inv);
        vec4 sample3 = texture(EntitySampler, Coord + vec2(TexelSize.x, -TexelSize.y) * inv);
        vec4 sample4 = texture(EntitySampler, Coord + vec2(-TexelSize.x, -TexelSize.y) * inv);
    ...
    return totalColor / (4.0 * maxSamples);
}
