...
import shiny.gildedglory.common.network.UpdateThrownWeaponStatusPayload;
...
public class ThrowableSwordComponent implements AutoSyncedComponent, ServerTickingComponent {
    ...
    public ThrowableSwordComponent(PlayerEntity provider) {
        this.provider = provider;
    }

    public static void clientTick(MinecraftClient client) {
        PlayerEntity player = client.player;
        ModComponents.THROWABLE_WIP.maybeGet(player).ifPresent(component -> {
            ...
            if (targeting != component.targeting) {
                ClientPlayNetworking.send(new UpdateThrownWeaponStatusPayload(targeting, false));
                component.targeting = targeting;
            }
            if (returning != component.returning) {
                ClientPlayNetworking.send(new UpdateThrownWeaponStatusPayload(returning, true));
...
//        }
        ...
        if (!this.isSummoned() && this.slot != -1) {
            if (this.iraedeusId != null) {
                ...
                if (!(entity instanceof ThrowableSwordEntity)) {
                    this.reset();
                }
            }
            else {
    ...
    public void setSlot(int slot) {
        this.slot = slot;
        ModComponents.THROWABLE_WIP.sync(this.provider);
    }
    ...
    public void returnStack() {
        ...
        if (bl) {
            this.reset();
            this.provider.getItemCooldownManager().set(ModItems.THROWABLE_WIP, 20);
        }
    }

    public void resetEntity() {
        Entity entity = GildedGloryUtil.getEntityFromUuid(this.iraedeusId, this.provider.getWorld());
        if (entity instanceof ThrowableSwordEntity iraedeus) {
    ...
    public Entity getTargetedEntity() {
        if (this.targetCooldown == 0 && !this.targeting) {
            Entity entity = GildedGloryUtil.getEntityFromUuid(this.iraedeusId, this.provider.getWorld());
            if (entity instanceof ThrowableSwordEntity || this.isSummoned()) {
